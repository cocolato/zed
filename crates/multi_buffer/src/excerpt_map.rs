...
use language::{
    ...
    OffsetUtf16, Point, PointUtf16, Selection, TextDimension, ToOffset as _, ToOffsetUtf16 as _,
    ToPoint as _, TransactionId, Unclipped,
};
...
impl ExcerptMap {
            ...
            if let Some(buffer) = this.as_singleton() {
                buffer.update(cx, |buffer, cx| {
                    buffer.edit(
                        edits
                            .into_iter()
                            .map(|(range, text)| (range.start.offset..range.end.offset, text)),
                        autoindent_mode,
                        cx,
                    );
                });
    ...
    fn convert_edits_to_buffer_edits(
        ...
        for (ix, (range, new_text)) in edits.into_iter().enumerate() {
            ...
            let start_overshoot = (range.start - *cursor.start()).offset;
            ...
            let end_overshoot = (range.end - *cursor.start()).offset;
            ...
            if let Some(buffer) = this.as_singleton() {
                buffer.update(cx, |buffer, cx| {
                    buffer.autoindent_ranges(
                        edits.into_iter().map(|e| e.0.start.offset..e.0.end.offset),
                        cx,
                    );
                });
                cx.emit(Event::ExcerptsEdited {
    ...
    pub fn insert_empty_line(
        ...
        if let Some(buffer) = self.as_singleton() {
            ExcerptMapPoint::with(buffer.update(cx, |buffer, cx| {
                buffer.insert_empty_line(point.point, space_above, space_below, cx)
            }))
        } else {
            ...
            point + ExcerptMapPoint::with(empty_line_start - buffer_point)
        }
    }
    ...
    pub fn insert_excerpts_with_ids_after<O: text::ToOffset>(
        ...
        self.edits = self.edits.compose([edit]);
            ...
            .filter_map(move |locator| {
                ...
                cursor.item().and_then(|excerpt| {
                    if excerpt.locator == *locator {
                        ...
                        let excerpt_end =
                            excerpt_start + ExcerptMapPoint::with(excerpt.text_summary.lines);
    ...
    pub fn offset_to_buffer_offset(
        ...
        cursor.item().map(|excerpt| {
            ...
            let buffer_offset = excerpt_start + (offset - *cursor.start()).offset;
    ...
    pub fn point_to_buffer_point(
        ...
        cursor.item().map(|excerpt| {
            ...
            let buffer_point = excerpt_start + (point - *cursor.start()).point;
            let buffer = self.buffers.borrow()[&excerpt.buffer_id].buffer.clone();
    ...
    pub fn range_to_buffer_ranges(
        ...
        while let Some(excerpt) = cursor.item() {
            ...
            if excerpt.has_trailing_newline {
                end_before_newline.offset -= 1;
            }
            let excerpt_start = excerpt.range.context.start.to_offset(&excerpt.buffer);
            let start = excerpt_start + (cmp::max(start, *cursor.start()) - *cursor.start()).offset;
            let end = excerpt_start + (cmp::min(end, end_before_newline) - *cursor.start()).offset;
            let buffer = self.buffers.borrow()[&excerpt.buffer_id].buffer.clone();
    ...
    pub fn remove_excerpts(
        ...
        while let Some(excerpt_id) = excerpt_ids.next() {
            ...
            if let Some(mut excerpt) = cursor.item() {
                ...
                if cursor.item().is_none() && old_start.offset > 0 {
                    old_start.offset -= 1;
    ...
    pub fn resize_excerpt(
        ...
        self.edits = self.edits.compose(edits);
        cx.emit(Event::Edited {
            singleton_buffer_edited: false,
            edited_buffer: None,
        });
    ...
    pub fn randomly_edit(
        ...
        for _ in 0..edit_count {
            ...
            let end = snapshot.clip_offset(
                ExcerptMapOffset::new(rng.gen_range(new_start.offset..=snapshot.len().offset)),
                Bias::Right,
            );
            let start = snapshot.clip_offset(
                ExcerptMapOffset::new(rng.gen_range(new_start.offset..=end.offset)),
                Bias::Right,
            );
            last_end = Some(end);
...
impl ExcerptMapSnapshot {
    ...
    pub fn reversed_chars_at(
        ...
        let mut excerpt_chunks = cursor.item().map(|excerpt| {
            ...
            let end = start + (cmp::min(offset, end_before_footer) - *cursor.start()).offset;
    ...
    pub fn clip_offset(&self, offset: ExcerptMapOffset, bias: Bias) -> ExcerptMapOffset {
        if let Some((_, _, buffer)) = self.as_singleton() {
            return ExcerptMapOffset::new(buffer.clip_offset(offset.offset, bias));
        }
        ...
        let overshoot = if let Some(excerpt) = cursor.item() {
            ...
            let buffer_offset = excerpt
                .buffer
                .clip_offset(excerpt_start + (offset - *cursor.start()).offset, bias);
    ...
    pub fn clip_point(&self, point: ExcerptMapPoint, bias: Bias) -> ExcerptMapPoint {
        if let Some((_, _, buffer)) = self.as_singleton() {
            return ExcerptMapPoint::with(buffer.clip_point(point.point, bias));
        }
        ...
        let overshoot = if let Some(excerpt) = cursor.item() {
            ...
            let buffer_point = excerpt
                .buffer
                .clip_point(excerpt_start + (point - *cursor.start()).point, bias);
            buffer_point.saturating_sub(excerpt_start)
        } else {
            Point::zero()
        };
        *cursor.start() + ExcerptMapPoint::with(overshoot)
    }
    ...
    pub fn bytes_in_range(&self, range: Range<ExcerptMapOffset>) -> MultiBufferBytes {
        ...
        let excerpt_bytes = if let Some(excerpt) = excerpts.item() {
            let mut excerpt_bytes = excerpt.bytes_in_range(
                (range.start - *excerpts.start()).offset..(range.end - *excerpts.start()).offset,
            );
    ...
    pub fn reversed_bytes_in_range(
        ...
        let excerpt_bytes = if let Some(excerpt) = excerpts.item() {
            let mut excerpt_bytes = excerpt.reversed_bytes_in_range(
                range.start.offset.saturating_sub(excerpts.start().offset)
                    ..range.end.offset - excerpts.start().offset,
            );
            chunk = excerpt_bytes.next().unwrap_or(&[][..]);
            Some(excerpt_bytes)
        } else {
    ...
    pub fn offset_to_point(&self, offset: ExcerptMapOffset) -> ExcerptMapPoint {
        if let Some((_, _, buffer)) = self.as_singleton() {
            return ExcerptMapPoint::with(buffer.offset_to_point(offset.offset));
        }
        ...
        if let Some(excerpt) = cursor.item() {
            ...
            let buffer_point = excerpt
                .buffer
                .offset_to_point(excerpt_start_offset + overshoot.offset);
            *start_point + ExcerptMapPoint::with(buffer_point - excerpt_start_point)
        } else {
            self.max_point()
        }
    }
    ...
    pub fn max_point(&self) -> ExcerptMapPoint {
        ExcerptMapPoint::with(self.excerpts.summary().text.lines)
    }
        ...
        D: TextDimension,
    {
        ...
        std::iter::from_fn(move || {
            ...
            if memoized_source_start.map_or(false, |_| point.point >= cursor.end(&()).lines) {
            ...
            if let Some(excerpt) = cursor.item() {
                let overshoot = point.point - cursor.start().lines;
    ...
    pub fn anchor_at(&self, position: ExcerptMapOffset, mut bias: Bias) -> Anchor {
        ...
        R: RangeBounds<ExcerptMapOffset>,
    {
        ...
        std::iter::from_fn(move || {
            if self.singleton {
                ...
                let row = MultiBufferRow(cursor.start().1.point.row);
...
impl ExcerptMapSnapshot {
    pub fn random_byte_range(
        ...
        let end = self.clip_offset(
            ExcerptMapOffset::new(rng.gen_range(start_offset.offset..=self.len().offset)),
            Bias::Right,
        );
        let start = self.clip_offset(
            ExcerptMapOffset::new(rng.gen_range(start_offset.offset..=end.offset)),
            Bias::Right,
        );
        start..end
    }
...
impl<'a> sum_tree::SeekTarget<'a, ExcerptSummary, TextSummary> for ExcerptMapPoint {
    fn cmp(&self, cursor_location: &TextSummary, _: &()) -> cmp::Ordering {
        Ord::cmp(&self.point, &cursor_location.lines)
    }
}
...
impl<'a> ExcerptMapRows<'a> {
    pub fn seek(&mut self, row: ExcerptMapRow) {
        ...
        self.excerpts
            .seek_forward(&ExcerptMapPoint::new(row.row, 0), Bias::Right, &());
        ...
        if let Some(excerpt) = self.excerpts.item() {
            let overshoot = row.row - self.excerpts.start().point.row;
...
impl<'a> Iterator for MultiBufferChunks<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.range.is_empty() {
            ...
            let excerpt = self.excerpts.item()?;
            self.excerpt_chunks = Some(excerpt.chunks_in_range(
                0..(self.range.end - *self.excerpts.start()).offset,
                self.language_aware,
            ));
            self.next()
        }
            ...
            if let Some(chunk) = self.excerpt_bytes.as_mut().and_then(|bytes| bytes.next()) {
                ...
                if let Some(excerpt) = self.excerpts.item() {
                    let mut excerpt_bytes =
                        excerpt.bytes_in_range(0..(self.range.end - *self.excerpts.start()).offset);
                ...
                if let Some(excerpt) = self.excerpts.item() {
                    let mut excerpt_bytes = excerpt.reversed_bytes_in_range(
                        self.range
                            .start
                            .offset
                            .saturating_sub(self.excerpts.start().offset)
                            ..usize::MAX,
                    );
                    self.chunk = excerpt_bytes.next().unwrap();
                    self.excerpt_bytes = Some(excerpt_bytes);
